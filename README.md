...
└── app/
    ...
    │   │   ├── agent.model.ts         # Interface Agent, AgentModel, ChatMessage, ChatRequest
    │   │   ├── conversation.model.ts  # Interface Conversation, ConversationMessage
    ...
    │       ├── agent.service.ts       # CRUD agents + agent chat API
    │       ├── conversation.service.ts # CRUD conversation + message history API
    │       ├── resource.service.ts    # CRUD resources + system prompt API
    │       ├── toast.service.ts       # Global toast notifications
    │       └── skill.service.ts       # CRUD skills + test API
    │
    ├── shared/
    │   └── components/
    │       ├── atoms/
...
| `TabChatComponent` | `tab-chat.component.ts` | Tab Chat: test agent, quản lý conversation history và nạp lại messages từ backend |
...
| Load conversations theo user | GET | `/api/conversations?userId={userId}` |
| Tạo / upsert conversation | POST | `/api/conversations` |
| Load messages của conversation | GET | `/api/conversations/{conversationId}/messages` |
| Xóa conversation | DELETE | `/api/conversations/{conversationId}` |
| Chat với agent | POST | `/v1/agents/chat` |

**Chat flow mới trong UI:**
- Tab Chat dùng `ConversationService` để tải danh sách conversation theo `userId = admin-ui`
- Khi bắt đầu hội thoại mới, UI sinh `conversationId` và `messageId` bằng `crypto.randomUUID()`
- Tin nhắn đầu tiên sẽ gọi `POST /api/conversations` để upsert conversation trước khi chat
- Sau đó UI gọi `POST /api/agents/chat` với `agentCode`, `conversationId`, `messageId`, `userId`, `messages`
- Khi chọn conversation cũ, UI gọi `GET /api/conversations/{conversationId}/messages` để khôi phục lịch sử
- Có thể xóa conversation hiện tại trực tiếp trong header của tab chat
...
agentSvc.chat({ agentCode, conversationId, messageId, userId, messages })
                                           // Observable<ChatResponse>
```

### `ConversationService`
Path: [src/app/core/services/conversation.service.ts](src/app/core/services/conversation.service.ts)

```typescript
conversationSvc.upsertConversation({ conversationId, userId, title })
                                           // Observable<Conversation>
conversationSvc.getConversations(userId)   // Observable<Conversation[]>
conversationSvc.getMessages(conversationId)
                                           // Observable<ConversationMessage[]>
conversationSvc.deleteConversation(id)     // Observable<void>
```
...
Hiện tại ứng dụng đã dùng API thực qua `HttpClient`. Khi trỏ sang backend khác:
...
2. Cập nhật `API_BASE_URL` trong [src/app/core/config/api.config.ts](src/app/core/config/api.config.ts) để đúng host backend.
3. Các service `AgentService`, `ConversationService`, `ResourceService`, `SkillService` sẽ gọi API trực tiếp theo cấu hình này.
